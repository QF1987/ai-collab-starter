...
| 维度 | main | **lite** |
|------|------|----------|
| Agent 角色 | Claude(架构) + Codex(实施) + OC(审) | **Codex(架构+拆任务+验收) + OC-impl(写) + OC-review(审) + OC-helper(信息查询) + Human(bus)** |
| 02-plan owner | Claude | **Codex** |
| 写代码 | Codex | **OC-impl** |
| Codex 是否写代码 | 是 | **否(03 期间不写,例外见下文 "Codex 临时写代码")** |
...
- `.ai/oc-code-quality-rubric.md`: Codex 03c 验收 OC-impl 产出的打分表(lite 专属)。
...
- `.ai/scratch/oc-helper/`: Codex ↔ OC-helper 共享文件总线(gitignored,临时)。
...
### Codex (lead engineer · T1)

- **职责**: 02 plan(架构 + 决策)+ 03a 拆任务 + 03c 验收 + 调度其他 Agent。
- **不写业务代码**(例外: 3 轮 verify fail 后 Human 走 (a) 临时授权)。
- 必须 force 出 `02-codex-plan.md` 强约束 7 条(alternatives ≥ 2 / pre-decisions ≥ 3 / OC delegation candidates 等)。
- 全仓搜索(无 path 限制)走 OC-helper,有限范围 Codex 自己。
- 验收用 `.ai/oc-code-quality-rubric.md`,门槛 ≥ 16/24。
- 第 3 轮 verify 仍 fail → 升 Human 决策,**不**自作主张接手。

### OC-impl (写代码主力 · T3)

- **职责**: 按 Codex 03a 子任务包写代码,产出 git working tree diff。
- 严格按子任务包执行,不越界。
- 严禁翻案 brief frontmatter 的 pre-decisions。
- 单文件 diff > 200 行 → 停下来问。
- 完成后输出 "done, 见 git diff",**不**自己总结(让 Codex 看 diff)。

### OC-review (独立审 · T4)

- **职责**: 04 三步法 + Codex 自审盲点专项检查。
- **必须**与 OC-impl 不同 session(强制隔离防自审盲点)。
- 严重 finding → escalate 给 Human,**不**给 Codex(避免 Codex 自审 OC-review 的产出)。

### OC-helper (信息查询 · T2,按需启动)

- **职责**: 全仓 grep / scan / summarize,结果写共享文件。
- 不动业务代码(只读全仓 + 写 `.ai/scratch/oc-helper/out-*.md`)。
- 触发: Codex 写 `req-*.md` 给指令,Human 切到 T2 让 OC-helper 跑 → OC-helper 写 out → Human 回 T1。
...
- 改动 <= 20 行 且 不跨模块 且 不涉及协议/接口 → Codex 可在 02 brief 中标 `human-direct: true`,Human 直接改不走 03 三段式。
- 改动 > 20 行 或 跨模块 或 涉及协议/接口 → 标准 03 三段式(03a → 03b → 03c → 04)。
- 纯格式化、依赖升级、机械性重命名 → 直接交 OC-impl,不消耗 Codex 验收槽。
- "broad refactor"(OC 禁止)定义: 跨 >= 3 个文件 或 跨模块边界 或 修改公共函数签名。

### Codex 临时写代码(例外路径)

03b ↔ 03c 走满 3 轮 verify 仍 fail → Human 决策三选(详见 `.ai/workflow.md §3c`):

- **(a) Codex 接手**: 此时 Codex 临时获得**本子任务包范围内**的写代码权限,改完 state.md 标注 `human-override-codex-fix`。
...
- Do not run unbounded full-repo scans unless the task explicitly requires it(此时走 OC-helper)。
...
- Use OC-helper for cheap full-repo context collection before Codex 02.
- Use Codex 02 only after the problem has a narrow question, decision point or risk list.
- Use OC-impl with bounded file paths, commands and acceptance criteria(子任务包)。
- End long conversations by writing a new `.ai/tasks/<task>.md` handoff.

## Review Rules

- Every non-trivial change requires review.
- OC-review focuses on three-step method: scope / architecture+pre-decisions / quality + Codex 自审盲点专项。
- Human handles escalation(architecture risk, protocol compatibility, security, rollback, long-term maintainability)。
- OC-impl fixes only approved findings and reruns targeted tests.
...
- Do not scan the entire monorepo repeatedly(走 OC-helper)。
- Do not merge generated summaries into source files unless requested.

### Agent Role Violations

- Do not let Codex perform bulk coding during 03(03 三段式之外的 commit 必须有 Human override 记录)。
- Do not let OC-impl redesign the whole system or翻案 pre-decisions。
- Do not let OC-review and OC-impl share session(强制隔离)。
...
>
> bootstrap 后此段几乎为空——随着项目跑通几个真实任务后由 Codex / OC 主动追加。
...
- 校验责任在 OC-review (`04-opencode-review.md > 文档状态翻转检查`)。

#### Patch artefact 完整性

- OC-impl / Codex(临时) 产出 `*.draft.patch` 时,必须:
...
#### OC-impl Scope 越界 pre-check

- OC-impl 实施前**必须**跑 `git diff --cached --stat`,逐行核对每个文件路径都在子任务包 paths 内。
- 出现范围外文件 → 立即停下回退,不要先跑测试再说。
- 越界检测责任在 OC-impl,审校方(Codex 03c + OC-review 04)兜底。

#### OC-review 与 OC-impl 共谋盲点(lite 特有)

- 因都是 OC(同模型 / 相似训练数据),**必须**强制 session 隔离。
- 同时 Codex 03c 已经先过一遍,OC-review 独立 cross-check。
- OC-review 三步法新增「Codex 自审盲点专项」checklist 兜底。
...
- 例外:调研类步骤(OC-helper packet / Codex ADR / 文档收口)不产代码改动时可标 `n/a`
...
> 所有 Agent 自然语言输出必须中文优先,避免 OC 国产模型自然倾向英文输出的失控。lite 中 3/4 Agent 都是 OC,纪律尤其重要。
