# Prompt: Lead Review 修复

## 角色

你是 Lead 修复已批准的 review finding。本次只做精确修复 + 验证。
...
为什么这么严：v1.0 实战中曾出现 Lead 在 RV "delete one line" 任务里把测试基础设施从 `@Container` 模式重构为 JDBC URL 模式，21 行删除 + 5 行新增。虽然结果可接受，但绕过了 Claude 审批 + workflow.md §6 "New architecture issues return to Claude" 被违反。详见 CHANGELOG / Finding 14。
...
epic-closeout 模式由调用 prompt 显式标 "本轮是 E1 epic-closeout 扫尾批次,scope 从单 slice paths 放宽为 epic 全域"。Reviewer verify 此类 fix 时采用简化模式：不做新一轮三步法，只验"RV 真修了 + 测试全过"。
...
1. **下一步 Agent**: `Reviewer | Lead | Human`
2. **关键输入**: 必读文件路径列表（≤ 4 条）
...
- 若所有 P0/P1 finding 已 fixed：输出 Reviewer re-review prompt（`04-review.md`）确认 verifier 状态翻 `verified`。
...
1. **必须**指向 Reporter 做 verify(`Next step.Agent` 字段值 = 该 RV 的 Reporter 字段值;对 `Reviewer` Reporter 适用)
2. `Prompt 模板` 字段:
   - Reporter = Reviewer → `.ai/prompts/04-review.md`
...
理由:v2.0 dogfood 二轮在 DeviceOps P1 #2 case 暴露——Lead 06-fix 完成后直接给 Human 翻 verified,
跳过 Reporter (Reviewer) 复审。当前 P2 case 走运,但 P0/P1 同样路径会违反 workflow.md "Reporter 翻 verified 才算关闭"
