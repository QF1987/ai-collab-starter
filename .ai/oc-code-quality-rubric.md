# 代码质量 rubric (lite v0.7.0-lite-rc1)

## 用途

Lead 03c 验收 Impl 产出时, 按本 rubric 打分。

**统一门槛: 总分 ≥ 16/24 通过**, 不分核心/glue。
(slice 类型在 brief 里 Lead 已标注, 当上下文给打分用, 不分门槛)

## 适用阶段

- Lead 03c 验收(主要场景)
- Reviewer 04 三步法第三步辅助参考(交叉验证 03c 是否偷工)
...
- [ ] **H1**. pre-decisions (brief frontmatter) 无一条被违反
  ...
  - 典型 fail 信号: D2 锁"不引入新依赖", Impl 在 package.json 加了 lodash
  - **(v0.4 · F05-self 新增) 子任务包必做段 override 03b 契约检测**: 子任务包"必做"段是否含 "刷 state.md" / "写下一步提示词" / "写 ADR" / "改 review.md" / "跑 04 自审" 等 03b 禁止项 → H1 直接 fail, 退回 Lead 03a 重写子任务包必做段
...
- [ ] **H3**. 编译 / lint / typecheck 通过 (Lead 本机跑)
...
- [ ] **H5**. 无可疑大段改动: 单文件 diff > 200 行 → 自动 fail (除非子任务包显式预声明)
  ...
  - **(v0.7 · F07-v0.7) verifier/test 脚本例外**: dedicated verifier / test 脚本 (非业务代码) 天然 gate 密集偏长 ——
    - 子任务包显式预声明了行数预算 → 按预算判 (业务代码 / verifier 脚本各自预算)
    - 子任务包未预声明 → verifier/test 脚本默认放宽到 250 行, 业务代码仍 200 行
    - 反例 (dogfood 留底): smart-uite `daemon-reader` / `h5coat-tray` 两个 epic 各因 verifier PowerShell 脚本 (208 行 / 205 行) 撞 200 cap 退回一轮, 03a 没主动预声明预算
...
- [hard fail 列表 / 或维度低分理由]
  e.g. H1 pre-decisions 违反: D2 锁"不引入新依赖", Impl 在 package.json 加了 lodash
...
- "顺手"重命名了无关变量
- (v0.2.0 · F16) **改了 config/*.ini / .env / config.yaml 等运行时配置**: Impl 跑测试撞墙顺手改, 是 lite v0.1 实战重灾区 (smart-uite Daemon 单例 bug 03b 第 1 轮就因此 fail)
- (v0.2.0 · F16) **改了 *.proto / migration/*.sql / 公共 header**: 架构敏感, 应升 Lead 02 重切
...
main v4.0-rc1 没有独立 rubric 文件, 用 04-review.md 的三步法做 quality 检查。
lite 引入独立 rubric 因为:

- Lead 03c 是 Lead **自审** Impl 产出, 需要更结构化的标准防漏检
- worker 跑国产模型, 训练数据偏弱, 需要更明确的"过线"门槛
...
- 退回模板"保留"段是否真的防 Impl 推倒重来——v0.2.0 加自动检测 (前后两轮 diff 重叠率)
