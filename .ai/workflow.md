...
- **强约束(v2.0)**：03 完成后，state.md 的 `Next step` **必须**指向 `04-opencode-review`，不允许直接跳到下一个 slice 的 03。例外仅在 task 文件显式标 `skip-review: true`（< 30 行单文件小补丁）时允许，且需注明豁免理由。

## 5. Review

Owner: OpenCode (single reviewer; Claude 在 escalation 时介入)。

### 5.1 OpenCode review 范围
...
OpenCode 在以下任一情况下**必须** escalate 给 Claude（state.md `Next step.Agent` 写 `Claude`，
...
无论 OpenCode review 还是 Claude escalation，所有 review 输出都必须落到 `.ai/review.md`。
...
- Claude escalation triggered on high-risk labels (v1.0 → v2.0: removed independent 05-claude-review step; Claude reviews are now escalations from OpenCode).
