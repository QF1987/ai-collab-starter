...
- `02-codex-plan.md > 收尾必做`: 新增 `#### 多分支 fallback 决策树 (P0/P1 必填)` 子段 — P0/P1 任务下一步提示词必填 ≥ 2 branch (happy + ≥ 1 fallback), 每 branch 完整 paste-able。
- `state.md > 维护规则 7`: 加 `(v0.6 · F01-v0.6 扩展)` 子块, 覆盖 5 个 fallback / iteration cross-terminal 切换点。
- `03-codex-orchestrate.md > 退回模板`: 加退回模板 prompt body force 约束 (state.md 必填完整 paste-able OC-impl 重试 prompt)。
