...
- `oc-helper.md`: 输出 chat 格式段 + Token 策略段加中文硬约束 (chat 散文/摘要默认中文, 结构字段名 + result 段命令原始输出例外; req `action.language: en` 例外)。
- `03b-opencode-impl.md > 输出格式` / `04-opencode-review.md > 输出`: 各加「chat 输出默认中文」约束。
- `AGENTS.md > Language Discipline`: 加 `### 国产 OC 模型实战覆盖` 子段 (lite framework 全局规则 + 临时覆盖手法 + req 例外)。
- 注: 结构字段名 `status` / `total_matches` 等保留英文 (工程惯例术语, Language Discipline 例外清单), 未按 finding 字面改 `状态:` — 避免破坏 Codex 对 out 文件结构字段解析。
